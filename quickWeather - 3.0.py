...
Dim ws As Worksheet: Set ws = wb.Sheets(2)
...
sht2.Range('A4').Value = 'Column B = Temperature in Kelvin'
sht2.Range('A5').Value = 'Column C = Temperature in Celcius'
...
for k,v in location.items():                  # For each country(k) and capital(v)...
    ...
    for k,v in w['main'].items():
        ...
        d = list(w['main'].values())          # Creating a list of the temperatures, pressures and humidity values and assigning to a variable 'd'
        dlist = d[1:4]                        # Selecting the 2nd to 4th values from that list
        sht.Range('C'+str(int(nrow3)+1)+':C'+str(int(nrow3)+3)).Value = [[temp-273.15] for temp in dlist] # Converting the temperatures from Kelvin to Degrees Celcius
    used = sht.UsedRange
    nrow = used.Row + used.Rows.Count-1
    nrow = nrow + 2
    nrow2 = nrow+1
